    ...
    <el-row>
      <el-col :offset="2" :span="20">
        ...
        <div>
          ...
          <div class="icon">
            <span class="iconfont icon-yinle1"><i>{{ singerInfo.musicSize }}</i></span>
            <span class="iconfont icon-zhuanjiguangpan"><i>{{ singerInfo.albumSize }}</i></span>
            <span class="iconfont icon-MV"><i>{{ singerInfo.mvSize }}</i></span>
          </div>
        </div>
        <!-- 歌手作品 -->
        <el-card>
            ...
            <el-tab-pane label="热门作品">
              <el-table
                ...
                <el-table-column label="时长" width="200px">
                  <template #default="scope">{{ scope.row.dt | secondFormate }}</template>
                </el-table-column>
                <el-table-column label="专辑">
                  <template #default="scope">
                    <span class="l_s_name" @click="saveAlbumId(scope.row.al.id)">
                      {{ scope.row.al.name }}
                    </span>
                  </template>
                </el-table-column>
...
<script>
import { getArtists, getSingerAl, getSingerMv, getSingerDesc } from "../../network/singer"; 
