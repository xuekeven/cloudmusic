  ...
  <div>
    <el-row :gutter="20">
      ...
      <el-col :offset="2" :span="4">
        <SingerType @emitHotSinger="getHotSingerEvent" @emitGetSinger="getSingerEvent" />
      </el-col>
      <!-- 右侧歌手展示区域 -->
      <el-col :span="16">
        <SingerShow :ListInfoAttr="ListInfoImg" v-if="isShow_3">
          <template #title>热门歌手</template>
          <template #more />
        </SingerShow>
        <div v-if="isShow_2">
          <SingerShow :ListInfoAttr="ListInfoImg">
            <template #title> {{ title }} </template>
            <template #query>
              <div class="letter-query" v-show="isShow_4">
                <a :class="active === index ? 'letter_bgc' : ''"
                  href=""
                  v-for="(item, index) in letter"
                  :key="index"
                  @click.prevent="letterQuery($event, index, item)">{{ item }}</a>
              </div>
            </template>
          </SingerShow>
        </div>
